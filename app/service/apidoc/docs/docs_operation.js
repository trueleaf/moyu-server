  ...
  async exportAsWord(params) {
    ...
    this.ctx.helper.dfsForest(nestDocs, (data, level) => {
      ...
      if (data.isFolder) { //文件夹
        ...
        try {
          this.ctx.helper.convertJSONToStringList(json5.parse(data.item.requestBody.rawJson)).forEach(str => {
        ...
        data.item.responseParams.forEach(res => {
          ...
          if (res.value.dataType === "application/json") {
            const jsonDoc = [];
            try {
              this.ctx.helper.convertJSONToStringList(json5.parse(res.value.strJson)).forEach(str => {
                jsonDoc.push(new Paragraph({
                  shading: {
                    type: ShadingType.SOLID,
                    color: "f3f3f3",
